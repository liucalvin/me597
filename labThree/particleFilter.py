...
class particleFilter(Node):
    ...
    def __init__(self, mapFilename="your_map/room.yaml", numParticles=500):
        ...
        self.mapUtilities = mapManipulator(mapFilename, laser_sig=0.5)
    ...
    def initializeParticleFilter(self, x, y, th):
        ...
        self.particlePoses = np.random.normal(
        [x, y, th], 
        [self.std_particle_x, self.std_particle_y, self.std_particle_theta],
        (numParticles, 3) 
        )
    ...
    def resample(self, laser_scan, mapUtilInstance):
        ...
        sampled_indices = np.random.choice(self.numParticles, size=self.numParticles, p=particles_weights)
